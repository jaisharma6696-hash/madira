  ...
  return (
    <div className="min-h-screen bg-brand-void">
      ...
      <motion.nav 
        ...
        className="fixed top-6 left-1/2 -translate-x-1/2 z-[90] flex items-center gap-4 md:gap-8 px-6 md:px-10 py-4 rounded-full bg-brand-navy/40 backdrop-blur-2xl border border-white/10 shadow-2xl"
      >
        <div className="hidden md:flex items-center gap-8">
          <a href="#brand" className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-cream/60 hover:text-white transition-colors">Story</a>
          <a href="#product" className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-cream/60 hover:text-white transition-colors">Portfolio</a>
        </div>
        
        <a href="#" className="text-base md:text-lg font-black gold-gradient tracking-[0.3em] mx-2 hover:scale-105 transition-transform">MADIRA</a>
        
        <div className="hidden md:flex items-center gap-8">
          <a href="#thesis" className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-cream/60 hover:text-white transition-colors">Thesis</a>
          <a href="#strategy" className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-gold hover:text-brand-gold-light transition-colors">Strategy</a>
        </div>
      </motion.nav>
      ...
      <footer className="py-16 md:py-24 border-t border-white/5">
        <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-end">
          <div className="space-y-8">
            <div className="font-serif text-3xl md:text-4xl font-black gold-gradient tracking-tight">
              {BRAND_NAME}
            </div>
            <p className="text-sm md:text-base text-brand-cream/40 max-w-sm leading-relaxed">
              A Himalayan craft spirits company reclaiming ancient heritage for the modern world.
            </p>
            <div className="flex gap-6">
              <a href="#" className="text-brand-gold/40 hover:text-brand-gold transition-colors"><ExternalLink size={20} /></a>
              <a href="#" className="text-brand-gold/40 hover:text-brand-gold transition-colors"><Users size={20} /></a>
            </div>
          </div>
          <div className="md:text-right space-y-4">
            <div className="text-[8px] md:text-[10px] font-bold uppercase tracking-[0.3em] text-brand-gold/40">
              Confidential · Investor Relations 2026
            </div>
            <div className="text-xs text-brand-cream/20">
              © 2026 Madira Spirits Pvt. Ltd. All rights reserved.
            </div>
          </div>
        </div>
      </footer>
      ...
      {/* Sticky Nav */}
      <nav className="fixed bottom-6 md:top-8 md:bottom-auto left-1/2 -translate-x-1/2 z-50 px-4 md:px-6 py-3 rounded-full bg-brand-void/80 backdrop-blur-xl border border-white/10 flex items-center gap-4 md:gap-8 shadow-2xl max-w-[95vw] md:max-w-none">
        <div className="font-serif font-black text-brand-gold tracking-tighter mr-2 md:mr-4">M</div>
        <div className="flex items-center gap-4 md:gap-6 overflow-x-auto hide-scrollbar">
          {['Brand', 'Product', 'Thesis', 'Strategy', 'Traction', 'Financials', 'Team', 'Invest'].map(item => (
            <a 
              key={item} 
              href={`#${item.toLowerCase()}`} 
              className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-brand-cream/40 hover:text-brand-gold transition-colors whitespace-nowrap"
            >
              {item}
            </a>
          ))}
